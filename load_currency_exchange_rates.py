...
def main():
    ...
    if conn is not None:
        ...
        data = pd.read_csv("data/yahoo_finance_currency_pairs.csv")
        ...
        for i,row in data.iterrows():
            try:
                ...
                print("row:", row)
                sql = "INSERT INTO currencyEX(currencyex_code, currencyex_name) VALUES (?, ?)"
                conn.execute(sql, tuple(row))
            except Exception as e:
                print("error inserting values :", tuple(row))
